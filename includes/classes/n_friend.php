...
class nFriend{
    ...
    public function cancel_agmt($agmt_id, $ret_id){
        $sql = "DELETE FROM `agreement` WHERE id = '$agmt'";
        if(mysqli_query($conn, $sql)){
            header("location: ../n2p_profile.php?id=$ret_id");
        }
    }
    
    public function acceptORreject($pid, $nid, $sd, $ed, $py, $req){
        if($req == "accept"){
            $sql = "INSERT INTO `agreement`(`pid`, `nid`, `start`, `end`, `payment`) VALUES ('$pid', '$nid', '$sd', '$ed', '$py')";
            if(mysqli_query($conn, $sql)){
                $sql1 = "UPDATE `agmt_status` SET `status`= 'accepted' WHERE `pid` = '$pid' AND `nid` = '$nid'";
                mysqli_query($conn, $sql1);
                $sql2 = "DELETE FROM `tmp_agmt` WHERE `pid` = '$pid' AND `nid` = '$nid'";
                mysqli_query($conn, $sql2);
                header("location: n_friends.php?id=$nid");
            }
        }
        else{
            $sql1 = "DELETE FROM `agmt_status` WHERE `pid` = '$pid' AND `nid` = '$nid'";
            mysqli_query($conn, $sql1);
            $sql2 = "DELETE FROM `tmp_agmt` WHERE `pid` = '$pid' AND `nid` = '$nid'";
            mysqli_query($conn, $sql2);
            header("location: n_friends.php?id=$nid");
    ...
    public function make_pending_friends($my_id, $user_id){
        ...
        try{
            ...
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$my_id, $user_id]);
            header('Location: n_profile.php?id='.$user_id);
            exit;
        }
        catch (PDOException $e) {
            die($e->getMessage());
                ...
                if($send_data){
                    ...
                    foreach($all_users as $row){
                        if($row->user_one == $my_id){
                            $get_user = "SELECT id, name, img FROM `patient` WHERE id = ?";
